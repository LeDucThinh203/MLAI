...
from sqlalchemy import Unicode, UnicodeText, String, Integer, DateTime, ForeignKey, Text
...
class Evidence(Base):
    ...
    stored_file_name: Mapped[str] = mapped_column(Unicode(255), nullable=False)
    mime_type: Mapped[str] = mapped_column(String(100), nullable=False)
    file_size: Mapped[int] = mapped_column(Integer, nullable=False)
    sha256_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    storage_path: Mapped[str] = mapped_column(Unicode(500), nullable=False)
    source_description: Mapped[Optional[str]] = mapped_column(UnicodeText, nullable=True)
